...
func ReleaseWebSocket(response http.ResponseWriter, request *http.Request) {
	...
	if conn != nil {
		services.EncounterWebSockets.UnbindClientWebSocketInEncounter(conn, request) // just in case the ws is associated with an encounter, drop the binding
...
func DebugWebSockets(response http.ResponseWriter, request *http.Request) {
	...
	encountersWebsockets := []encounterWebsockets{}
	for encounterId, conns := range services.EncounterWebSockets.ConnsByEncounterId {
		var remoteAddrs []string
		for _, conn := range conns {
			remoteAddrs = append(remoteAddrs, conn.RemoteAddr().String())
		}
		encountersWebsockets = append(encountersWebsockets, encounterWebsockets{EncounterId: encounterId, RemoteAddrs: remoteAddrs})
	}

	type data struct {
		Websockets           []websocket           `json:"websockets"`
		EncountersWebsockets []encounterWebsockets `json:"encountersWebsockets"`
	}
	WriteJsonResponse(response, http.StatusOK, data{Websockets: websockets, EncountersWebsockets: encountersWebsockets})
}
